...
def questions_states():
  states_dict: dict[int, any] = {}    
                          
  def regex(question: Question):
    return ''.join(f"{re.escape(x.name)}|" for x in question.answers)[:-1]
  
  for index, question in enumerate(questions):
    states_dict[index] = [MessageHandler(filters=filters.Regex(f"^({regex(question)})$"), callback=test_question_command)]
